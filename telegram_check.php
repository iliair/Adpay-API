...
$telegram_qry="SELECT * FROM telegram_check";
$telegram_result=mysqli_query($mysqli,$telegram_qry);
if (mysqli_num_rows($telegram_result) > 0) {
  $x =0;
    // output data of each row
    while($row = mysqli_fetch_assoc($telegram_result)) {
      ...
      echo '<form name="vidform'.$x.'" id="vidform'.$x.'" action="submit_telegram.php" method="post">';
